...
for i = 1:length
   if notes_match(i) ~= -1
      if  notes_match(i) == 1 || notes_match(i) == 2
          note_length(clef, note_counter) = 4;
          num = round((centers(line_counter)-center_points(i, 2))/unit);
          if clef == 1
              num = num + 7;
              song(clef, note_counter) = notes_1(num);
          else
              num = num+8;
              num = 15-num;
              song(clef, note_counter) = notes_2(num);
          end
          note_counter = note_counter+1;
      elseif notes_match(i) == 3 || notes_match(i) == 4 || notes_match(i) == 5 || notes_match(i) == 6
          note_length(clef, note_counter) = 2;
          num = round((centers(line_counter)-center_points(i, 2))/unit);
          if clef == 1
              num = num + 7;
              song(clef, note_counter) = notes_1(num);
      ...
      elseif notes_match(i) == 7 || notes_match(i) == 8 || notes_match(i) == 9 || notes_match(i) == 10
          note_length(clef, note_counter) = 1;
          num = round((centers(line_counter)-center_points(i, 2))/unit);
          if clef == 1
              num = num + 7;
              song(clef, note_counter) = notes_1(num);
      ...
      elseif notes_match(i) == 13
          note_length(clef, note_counter) = 0.5;
          song(clef, note_counter) = 'Q';
          note_counter = note_counter+1; 
      elseif notes_match(i) == 14
          note_length(clef, note_counter) = 0.25;
          song(clef, note_counter) = 'Q';
          note_counter = note_counter+1; 
